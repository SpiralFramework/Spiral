...
public abstract class BaseSpiralPlugin protected constructor(
    public val context: SpiralCoreContext,
    public val callingClass: Class<*>,
    public val resourceName: String,
    public val yaml: Boolean = true
) : ISpiralPlugin {
    public lateinit var pojo: SpiralPluginDefinitionPojo
    ...
    private val jarFile =
        callingClass::class.java.protectionDomain.codeSource?.location?.path?.let(::File)?.takeIf(File::isFile)
    override val dataSource: DataSource<*> =
        jarFile?.let { f -> AsyncFileDataSource(f) } ?: BinaryDataSource(byteArrayOf())
