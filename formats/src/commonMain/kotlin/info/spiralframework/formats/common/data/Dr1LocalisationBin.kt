    ...
    companion object {
        ...
        suspend fun unsafe(context: SpiralContext, flow: InputFlow): Dr1LocalisationBin {
            withFormats(context) {
                ...
                val languageCount = requireNotNull(flow.readInt32LE(), notEnoughData).toInt()
                val languages = Array(languageCount) {
                    val languageIndex = requireNotNull(flow.readInt32LE(), notEnoughData)
                ...
                return Dr1LocalisationBin(languages.first { lang -> lang.languageID == 0x00 }.localisations, languages.map { lang -> lang.languageName to lang }.toMap())
            }
        }
    }
}