...
class WordScriptFile private constructor(context: SpiralContext, val game: V3, val dataSource: () -> InputStream) {
    companion object {
        operator fun invoke(context: SpiralContext, game: V3, dataSource: () -> InputStream): WordScriptFile? {
            withFormats(context) {
                try {
                    return WordScriptFile(this, game, dataSource)
                } catch (iae: IllegalArgumentException) {
                    debug("formats.wrd.invalid", dataSource, game, iae)
        ...
        fun unsafe(context: SpiralContext, game: V3, dataSource: DataSource): WordScriptFile = withFormats(context) { WordScriptFile(this, game, dataSource) }
    }
        ...
        with(context) {
            ...
            try {
                ...
                entries = Array(labelCount) { index ->
                    ...
                    if (size < 0) {
                        throw IllegalArgumentException(localise("formats.wrd.bad_size", index, size))
                    } else if (sectionOffsets[index] <= 0) {
                        throw IllegalArgumentException(localise("formats.wrd.bad_offset", index, sectionOffsets[index]))
                    }
...
//                        }
                            ...
                            if (arguments.size != argumentCount / 2 && argumentCount != -1) {
                                warn("formats.wrd.wrong_arg_count", "0x${opCode.toString(16)}", argumentCount, arguments.size)
                            }
