...
object CRILAYLACompression: ICompression {
    ...
    override fun isCompressed(context: SpiralContext, dataSource: () -> InputStream): Boolean = dataSource().use { stream -> stream.readInt64LE() == MAGIC_NUMBER }
    override fun decompressToPipe(context: SpiralContext, dataSource: () -> InputStream, sink: OutputStream) {
        sink.write(decompress(context, dataSource))
    }

    override fun decompress(context: SpiralContext, dataSource: () -> InputStream): ByteArray =
            dataSource().use { stream ->
                ...
                return@use decompress(context, uncompressedSize, dataSize, compressedData, rawDataHeader)
            }

    /** This code is mostly unmodified, so just pray that it works */
    fun decompress(context: SpiralContext, uncompressedSize: Int, dataSize: Int, compressedData: ByteArray, dataHeader: ByteArray): ByteArray {
