...
class LargeHttpClient(val proxy: Proxy? = null) {
    companion object {
        private val INSTANCES: MutableMap<Proxy?, LargeHttpClient> = HashMap()

        operator fun get(proxy: Proxy?): LargeHttpClient {
            if(proxy in INSTANCES)
                return INSTANCES[proxy]!!

            val instance = LargeHttpClient(proxy)
    ...
    fun executeRequest(request: Request): LargeResponse {
        val connection = establishConnection(request) as HttpURLConnection

        try {
            connection.apply {
                connectTimeout = Fuel.publicTestConfiguration.coerceTimeout(request.timeoutInMillisecond)
                readTimeout = Fuel.publicTestConfiguration.coerceTimeoutRead(request.timeoutReadInMillisecond)
                doInput = true
                useCaches = false
    ...
    private fun establishConnection(request: Request): URLConnection {
        ...
        return if (request.url.protocol == "https") {
            ...
            conn.apply {
                sslSocketFactory = request.publicSocketFactory
                hostnameVerifier = request.publicHostnameVerifier
            }
        } else {
