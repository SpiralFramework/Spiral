...
data class CPKFileEntry(val fileName: String, val directoryName: String, val fileSize: Long, val extractSize: Long, val offset: Long, val isCompressed: Boolean, val cpk: CPK) : DataSource {
    ...
    override val data: ByteArray
        get() = if(isCompressed) CRILAYLAFormat.convertToBytes(SpiralFormat.BinaryFormat, ByteArrayDataSource(rawData), emptyMap()) else rawData
    ...
    val rawData: ByteArray
        get() = OffsetInputStream(cpk.dataSource.inputStream, offset, fileSize).use { it.readPartialBytes(size.toInt()) }
}